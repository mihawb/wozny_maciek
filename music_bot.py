...
class Music(commands.Cog):
    ...
    async def play(self, ctx, *, query):
        ...
        await ctx.send('Z radyjka leci: {}'.format(query))
    ...
    async def stream(self, ctx, *, url):
        ...
        await ctx.send('Z radyjka leci: {}'.format(player.title))

    @commands.command()
    async def volume(self, ctx, volume: int):
        if ctx.voice_client is None:
            return await ctx.send("Nie słuchasz nawet, na głosowy najpierw wejdź.")
    ...
    async def ensure_voice(self, ctx):
        if ctx.voice_client is None:
            if ctx.author.voice:
                ...
                await ctx.send("A na głosowy wejdź pierw.")
...
@bot.event
async def on_ready():
    print('Logged in as {0} ({0.id}) from music_bot'.format(bot.user))
    print('------')
